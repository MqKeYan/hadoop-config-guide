...
5. **环境变量设置**  
   接着输入`exit`退出 Root 账户，输入`vim /home/user/.bash_profile`（注意将`/home/user/`中的`user`改为实际的系统用户名）进入系统默认用户的环境变量文件编辑界面，先按下键盘上的`i`键进入编辑状态，再根据编辑文件详情在文件末尾加上，完成后先按下键盘上的ESC键，再输入:wq后按回车键进行保存（注意:为英文符号）。操作如图：
<p align="center">
  <img src="../../image/Java%20Configuration%20Diagram/P.png" width="60%">
</p>
