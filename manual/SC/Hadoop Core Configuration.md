...
3. **配置 yarn-site.xml 文件**  
   输入`vim $HADOOP_HOME/etc/hadoop/yarn-site.xml`编辑`yarn-site.xml`文件，先按下键盘上的`i`键进入编辑状态，再根据[yarn-site.xml 文件](../../notes/home/user/hadoop-3.4.2/etc/hadoop/yarn-site.xml)编辑文件，完成后先按下键盘上的`ESC`键，再输入`:wq`后按回车键进行保存。然后输入`hdfs dfs -mkdir -p /tmp/logs`创建目录，再输入`hdfs dfs -chmod -R 1777 /tmp/logs`设置权限。操作如图：  
   **注：**
   注意不要复制`yarn-site.xml`文件里面的注释，创建的文件夹地址改为实际需要创建的。`创建数据储存目录`这个步骤需要在配置完成并且启动 HDFS 后再执行，否则会报错，因为要启动 HDFS 后才能在 HDFS 上创建文件夹。
<p align="center">
...
5. **配置 workers 文件**  
   ...
   **注：**
   在`Hadoop 3.x`中将原本的`slaves`改为了`workers`，不要复制`workers`文件里面的注释，创建的文件夹地址改为实际需要创建的地址。
<p align="center">
  <img src="../../image/Hadoop%20Core%20Configuration%20Diagram/P6.png" width="40%">
</p>
