...
**注**：软件安装过程出现问题可以去讨论区发帖，也可以咨询 Ai 获取解决方法（推荐使用国外 Ai 的模型）。

## 系统配置
- **虚拟机克隆**  
  通过克隆第1台虚拟机创建出额外2个虚拟机，通过这3台虚拟机组成集群，以此来搭建 Hadoop 集群。具体步骤请参考[虚拟机克隆详情](manual/.md)。
- **虚拟网络配置**  
  在 VMware Workstation 中通过`虚拟网络编辑器`配置虚拟机的IP，确保虚拟机能够正常访问外部网络。具体步骤请参考[虚拟网络配置详情](manual/.md)。
- **集群基础配置**  
  将第1台虚拟机作为集群的主机，其余2台虚拟机作为集群的子机，然后对虚拟机进行集群基础配置。具体步骤请参考[集群基础配置详情](manual/.md)。
- **文件导入**  
  使用Xftp将`Hadoop`、`Spark`和`Java`等的压缩包传入虚拟机根目录。具体步骤请参考[文件导入详情](manual/.md)。
- **SSH 配置**  
  通过配置 SSH 可以实现虚拟机节点之间的 SSH 无密码登录，简化集群管理操作流程同时提高运维效率。具体步骤请参考[SSH 配置详情](manual/.md)。
- **Eclipse 安装**  
  在主机上安装 Eclipse 集成开发环境，用于后续 Hadoop 与 Spark 项目的开发、调试与运行。[Xshell 安装详情](manual/.md)。

**注**：系统配置过程出现问题可以去讨论区发帖，也可以咨询 Ai 获取解决方法（推荐使用国外 Ai 的模型）。

## Hadoop 集群搭建

- **Hadoop 安装**  
  在各虚拟机节点上安装 Hadoop 并配置相应的运行环境变量，确保 Spark 与已部署的 Java 版本兼容。具体步骤请参考[Hadoop 安装详情](manual/.md)。
- **Hadoop 基础配置**  
...
- **Spark 安装**  
  在各虚拟机节点上安装 Spark 并配置相应的运行环境变量，确保 Spark 与已部署的 Hadoop 版本兼容。具体步骤请参考[Spark 安装详情](manual/.md)。
- **Spark 基础配置**  
...
欢迎在 GitHub 中提出问题、分享想法或讨论改进方案：[讨论入口](../../discussions)
