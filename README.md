...
## 系统配置
- **虚拟机克隆**  
  通过克隆第1台虚拟机创建出额外2个虚拟机，以此来搭建 Hadoop 集群。具体步骤请参考[虚拟机克隆详情](manual/.md)。
- **虚拟网络配置**  
  在 VMware Workstation 中通过`虚拟网络编辑器`配置服务器的IP，确保虚拟机能够正常访问本地 PC 的网络。具体步骤请参考[虚拟网络配置详情](manual/.md)。
- **集群基础配置**  
